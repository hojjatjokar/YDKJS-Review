...
6. ### What does `Object.prototype` include?
...
7. ### How is setting property on object if property doesn't exist on object?
...
9. ### What are 3 scenarios for the **myObject.foo = "bar"** assignment when foo is not already on `myObject` directly, but is at a higher level of `myObject`'s `[[prototype]]` chain?
...
- If it has setter, the setter will be called
...
b. `Foo.prototype.myName = ...`: perhaps the more interesting technique, this adds a property (function) to the `Foo.prototype` object. Now, `a.myName()` works, but perhaps surprisingly.
...
27. ### Why the words "constructor" and "prototype" only have a loose default meaning?
